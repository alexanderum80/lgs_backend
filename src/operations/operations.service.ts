...
export class OperationsService {
  constructor(
    @InjectRepository(OperationsREntity)
    private readonly operationRRepository: Repository<OperationsREntity>,
    @InjectRepository(OperationsDEntity)
    private readonly operationDRepository: Repository<OperationsDEntity>,
  ...
  async findAllToday(idOperationType: number): Promise<OperationsRView[]> {
    try {
      const today = await this._casinoInfoSvc
        .findCasinoOpeningDate()
        .catch((err) => {
      ...
      return new Promise<OperationsRView[]>((resolve, reject) => {
        getManager()
          .query(
            `select * from "vw_OperationsR" where "IdOperationType" = ${idOperationType} and "Date" >= '${today.toDateString()}'`,
          )
          .then((result) => {
  ...
  async findOne(id: number): Promise<OperationsREntity> {
    try {
      return new Promise<OperationsREntity>((resolve, reject) => {
        this.operationRRepository
          .findOne(id, { relations: ['OperationsD'] })
          .then((result) => {
            resolve(result);
          })
          .catch((err) => {
  ...
  async create(
    ...
    try {
      ...
      operationInput.OperationR.Consecutive =
        await this.getInstrumentConsecutive(
          operationInput.OperationR.IdOperationType,
        );
      ...
      return new Promise<OperationsREntity>((resolve, reject) => {
        this.operationRRepository
          ...
          .then((result) => {
            ...
            this.operationDRepository
              .delete({ IdOperation: operationInput.OperationR.IdOperation })
              .then(() => {
                this.operationDRepository
                  .save(operationInput.OperationD)
                  .then(() => {
  ...
  async finishOperation(idOperation: number): Promise<number> {
    try {
      return new Promise<number>(async (resolve, reject) => {
        getManager()
          .findOne(OperationsRView, { IdOperation: idOperation })
          .then((res) => {
            if (
              res.IdOperationType === EOperations.CREDIT &&
              res.AmountIn !== res.AmountOut
            ) {
              return reject(
                'Approved Amount do not match with Delivered Amount. Fix it.',
              );
            }

            this.operationRRepository
              .update({ IdOperation: idOperation }, { Finished: true })
              .then((result) => {
                this._insertOperationInCage(idOperation)
                  .then(() => {
  ...
  async cancelOperation(idOperation: number): Promise<number> {
    try {
      return new Promise<number>((resolve, reject) => {
        this.operationRRepository
          .update({ IdOperation: idOperation }, { Cancelled: true })
          .then((result) => {
            this._cageSvc
              .remove(idOperation)
              .then(() => {
      ...
      return new Promise<boolean>(async (resolve, reject) => {
        ...
        for (let index = 0; index < tables.length; index++) {
          ...
          operationR = {
            ...
            Consecutive: await this.getInstrumentConsecutive(
              EOperations.INITIALIZING,
            ),
            IdUser: user,
            Finished: true,
            Date: new Date(),
          ...
          await this.operationRRepository
            .save(operationR)
            .then(async (result) => {
        ...
        operations.forEach(async (o) => {
          await getManager()
            .getRepository(OperationsREntity)
            .createQueryBuilder()
            .update()
        ...
        await this._casinoInfoSvc
          .updateCasinoState(EOperations.OPEN, operationDate)
          .then(() => {
            resolve(true);
          })
          .catch((err) => {
  ...
  async finishClosing(): Promise<boolean> {
    try {
      return new Promise<boolean>(async (resolve, reject) => {
        const pendingOperations = await this.operationRRepository
          .find({ Finished: false })
          .then((op) => {
            return op;
          });
        ...
        if (pendingOperations.length) {
          reject(
            `There are ${pendingOperations.length} operations that aren't Finished. You must Finish or Delete those operations.`,
          );
          return;
        }

        await this._casinoInfoSvc
          .updateCasinoState(EOperations.CLOSED)
          .then(() => {
            resolve(true);
          })
          .catch((err) => {
  ...
  async getInstrumentConsecutive(idOperation: number): Promise<number> {
    return new Promise<number>(async (resolve, reject) => {
      getManager()
        .query(`SELECT fn_lgs_getopnumber(${idOperation});`)
        .then((res) => {
  ...
  private async _insertOperationInCage(idOperation: number): Promise<boolean> {
    return new Promise<boolean>(async (resolve, reject) => {
      getManager()
        .query(`CALL public.sp_lgs_insert_cage(${idOperation})`)
        .then((result) => {
          resolve(result);
        })
        .catch((err) => {
